...
int type_of_move(char field[][WIDTH], char buffer[], int cur_sym) {
  if (buffer[cur_sym + 2] == 'x') {
    if (!(islower(field[number(buffer[cur_sym + 4])]
                       [letter(buffer[cur_sym + 3])]) ==
          islower(
...
int is_right_field_range(char buffer[], int cur_sym) {
  if (number(buffer[cur_sym + 4]) >= LENGTH ||
      letter(buffer[cur_sym + 3]) >= WIDTH) {
...
int is_correct_figure(char field[][WIDTH], char buffer[], int cur_sym,
                      char figure) {
...
int move_pawn(char field[][WIDTH], char buffer[], int cur_sym) {
  ...
  if (type_of_move(field, buffer, cur_sym) == 1) // attack peshki
  {
    ...
    if (figure == 'P') {
      if (abs_num == 2 &&
          white_pawns_first_move[letter(buffer[cur_sym])] == 0 &&
        ...
        field[number(buffer[cur_sym + 4])][letter(buffer[cur_sym + 3])] =
            figure;
      } else if (abs_num == 1 && (field[number(buffer[cur_sym + 1]) + 1]
                                       [letter(buffer[cur_sym])] == ' ')) {
        field[number(buffer[cur_sym + 1])][letter(buffer[cur_sym])] = ' ';
        field[number(buffer[cur_sym + 4])][letter(buffer[cur_sym + 3])] =
            figure;
      } else {
      ...
      if (abs_num == 2 &&
          black_pawns_first_move[letter(buffer[cur_sym])] == 0 &&
          (field[number(buffer[cur_sym + 1]) - 1][letter(buffer[cur_sym])] ==
           ' ') &&
          (field[number(buffer[cur_sym + 1]) - 2][letter(buffer[cur_sym])] ==
           ' ')) { // for black pawn
...
int move_queen(char field[][WIDTH], char buffer[], int cur_sym) {
  ...
  } else if ((abs_num == 0 && abs_let != 0) ||
             (abs_num != 0 && abs_let == 0)) { // rock
    if (direction_num > 0) {
      for (int i = 1; i <= abs_num; i++) {
